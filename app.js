...
function setApiKey(k) { localStorage.setItem(KEY_OPENAI, k.trim()); }
function clearApiKey() { localStorage.removeItem(KEY_OPENAI); }
...
function renderBriefs() {
  ...
  var keyStatus = hasKey
    ? '<div class="api-status">' +
        '<span class="chip chip-brief">AI ready</span>' +
        '<button class="btn-ghost btn-small" id="btn-clear-key">Remove key</button>' +
      '</div>'
    : '<div class="api-status api-status-warn">' +
        '<span>No OpenAI key set — will use offline generator.</span>' +
        '<button class="btn-ghost btn-small" id="btn-show-key-form">Add API key →</button>' +
        '<div id="api-key-form" class="hidden" style="display:none;margin-top:10px;display:flex;gap:8px;align-items:center">' +
          '<input type="password" id="api-key-input" placeholder="sk-..." style="flex:1">' +
          '<button class="btn-primary btn-small" id="btn-save-key">Save</button>' +
        '</div>' +
      '</div>';
  ...
  return '<div class="brief-builder">' +
    ...
    '<div class="card">' +
      '<h2>Brief Builder</h2>' +
      '<p class="subtitle">Generate a structured argument, memo, or essay outline — powered by AI when a key is set.</p>' +
      keyStatus +
...
function wireBriefs() {
  /* ── API key UI ── */
  var showKeyBtn = document.getElementById('btn-show-key-form');
  if (showKeyBtn) {
  ...
  document.getElementById('btn-generate').addEventListener('click', function() {
    ...
    if (apiKey) {
      generateBriefWithAI(prompt, notes, briefType, tone, apiKey).then(function(result) {
...
function buildPromptForModel(prompt, sourceNotes, briefType, tone) {
  var notesSection = sourceNotes.trim()
    ? 'Source notes from the student (use these to shape the content):\n' + sourceNotes.trim()
    : 'Source notes: none provided.';

  return 'You are helping a student create a structured academic brief.\n\n' +
    'Prompt or question: ' + prompt.trim() + '\n\n' +
    notesSection + '\n\n' +
    'Brief type: ' + briefType + '\n' +
    'Desired tone: ' + tone + '\n\n' +
    'TASK: Produce a fully tailored brief — not a generic template. Use the prompt and notes directly to shape every sentence. ' +
    'Vary sentence structure; avoid starting multiple paragraphs with the same word or phrase.\n\n' +

    (briefType === 'Debate Case' ?
    'Write the following sections:\n' +
    'Thesis: (2–3 sentences taking a clear, specific side on the motion. Do not say "Argue that" — just state the position.)\n\n' +
    'Contention 1 — [Short descriptive title]: (4–6 sentences. Start with a bold claim, then build with reasoning and specific impact. End with what this means for decision-makers.)\n\n' +
    'Contention 2 — [Short descriptive title]: (4–6 sentences. Use a different angle from Contention 1. Weave in any keywords from the notes.)\n\n' +
    'Contention 3 — [Short descriptive title]: (4–6 sentences. Cover a democratic, historical, or institutional dimension.)\n\n' +
    'Counterargument 1 + Response: (Write a realistic objection — not just "critics argue." Then directly rebut it using the contentions or notes.)\n\n' +
    'Counterargument 2 + Response: (A second, different objection. Rebut it concisely.)\n\n' +
    'Conclusion: (2–3 sentences. Tie the three contentions together and state what a judge or decision-maker should do.)\n\n' +
    'Research Plan: (3–5 specific suggestions for evidence to find — name the type of study, case, report, or expert. Be concrete, not generic.)\n'
    : briefType === 'Policy Memo' ?
    'Write the following sections:\n' +
    'Executive Summary: (3–5 sentences. State the issue, your recommendation, and the top reason.)\n\n' +
    'Background: (3–5 sentences. Give real context; weave in any keywords from notes. Use "Data on [X] suggests…" style phrasing.)\n\n' +
    'Problem Statement: (2–4 sentences. Be specific about who bears the costs and why the status quo fails.)\n\n' +
    'Policy Options: (List 2–3 real options with a line of pros/cons each. Name them concretely, not just "Option A/B/C".)\n\n' +
    'Recommendation: (2–4 sentences. Pick one option and justify it with evidence from the background.)\n\n' +
    'Next Steps: (3 concrete bullet points — specific actions, not vague directions. Include a timeline hint.)\n\n' +
    'Research Plan: (3–5 specific suggestions for evidence to find — studies, reports, legal cases, expert voices.)\n'
    :
    'Write the following sections:\n' +
    'Hook: (1–3 vivid, scene-setting sentences. No "This essay will…" openings. Put the reader inside a moment.)\n\n' +
    'Core Story: (One paragraph, 4–6 sentences. Describe the situation, conflict, or project at the heart of the essay.)\n\n' +
    'Key Moment 1 — [Short title]: (1–3 sentences. A concrete beat in the story.)\n' +
    'Key Moment 2 — [Short title]: (1–3 sentences. A complication or turning point.)\n' +
    'Key Moment 3 — [Short title]: (1–3 sentences. A realization or outcome.)\n\n' +
    'Reflection: (One paragraph, 3–5 sentences. What changed inside the student — beliefs, habits, goals.)\n\n' +
    'Takeaway: (2–3 sentences. Connect the experience to what the student will bring to college. Be specific, not grandiose.)\n\n' +
    'Research Plan: (3–4 suggestions — specific essay prompts to consider, examples of successful essays with similar structures, or self-reflection questions to deepen the story.)\n'
    ) +
    '\nWrite everything as plain text with the section names as headings followed by a colon, then a blank line, then the content. ' +
    'Do not use Markdown bullets, asterisks, or code fences. Use plain dashes for any list items.';
}

async function generateBriefWithAI(prompt, sourceNotes, briefType, tone, apiKey) {
  var systemPrompt = 'You are an expert debate coach, policy analyst, and college essay mentor. ' +
    'You help students turn prompts and rough notes into polished, specific, well-structured briefs. ' +
    'Your output is always direct, substantive, and tailored to the exact prompt — never generic or templated.';

  var userPrompt = buildPromptForModel(prompt, sourceNotes, briefType, tone);
