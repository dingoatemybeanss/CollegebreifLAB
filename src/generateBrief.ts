import type { BriefType, Tone } from "./firebase";

export type { BriefType, Tone };
...
function extractKeywords(notes: string, prompt: string): string[] {
  ...
  return Object.entries(freq).sort((a, b) => b[1] - a[1]).map(([w]) => w).slice(0, 5);
}
...
const T = {
  neutral: { open: "The evidence suggests", argue: "This brief finds", note: "It is worth noting that", conclude: "The weight of evidence supports", data: "Available data on" },
  advocacy: { open: "This brief argues", argue: "We must recognize", note: "Critically,", conclude: "Decision-makers must act on", data: "Documented evidence on" },
  academic: { open: "This paper contends", argue: "The available literature indicates", note: "It bears noting that", conclude: "The preponderance of scholarly evidence supports", data: "Empirical research on" },
} as const;

function buildDebate(topic: string, kws: string[], tone: Tone): DebateSections {
  const t = T[tone];
  const topicLow = topic.toLowerCase();
  const topicWords = topic.replace(/[^a-z0-9\s]/gi, " ").split(/\s+/).filter((w) => w.length >= 4 && !STOPS.has(w.toLowerCase()));
  ...
  const conclusion = tone === "academic"
...
function buildEssay(topic: string, kws: string[], tone: Tone, sourceNotes: string): EssaySections {
  ...
  const reflection = tone === "academic"
...
export function generateBrief(prompt: string, sourceNotes: string, briefType: BriefType, tone: Tone): GeneratedBrief {
  ...
  if (briefType === "debate") {
    ...
    sections = d;
    plainText = ["DEBATE CASE", `Motion: ${cleanPrompt}`, `Tone: ${toneLabel}`, "", "THESIS", d.thesis, "", ...d.contentions.flatMap((c, i) => [`CONTENTION ${i + 1} — ${c.title.toUpperCase()}`, c.body, ""]), "COUNTERARGUMENTS & RESPONSES", ...d.counterarguments.flatMap((ca, i) => [`CA ${i + 1}: ${ca.counter}`, `Response: ${ca.response}`, ""]), "CONCLUSION", d.conclusion].join("\n");
  } else if (briefType === "policy") {
    const p = buildPolicy(topic, kws, tone, cleanNotes);
    sections = p;
    plainText = ["POLICY MEMO", `Subject: ${cleanPrompt}`, `Tone: ${toneLabel}`, "", "EXECUTIVE SUMMARY", p.executiveSummary, "", "BACKGROUND", p.background, "", "PROBLEM STATEMENT", p.problem, "", "POLICY OPTIONS", ...p.options.flatMap((o) => [`${o.label}`, o.description, ""]), "RECOMMENDATION", p.recommendation, "", "NEXT STEPS", p.nextSteps].join("\n");
  } else {
    const e = buildEssay(topic, kws, tone, cleanNotes);
    sections = e;
    plainText = ["COLLEGE ESSAY OUTLINE", `Prompt: ${cleanPrompt}`, `Tone: ${toneLabel}`, "", "HOOK", e.hook, "", "CORE STORY", e.coreStory, "", "THREE MOMENTS", ...e.moments.map((m) => `• ${m}`), "", "REFLECTION", e.reflection, "", "TAKEAWAY", e.takeaway].join("\n");
  }
